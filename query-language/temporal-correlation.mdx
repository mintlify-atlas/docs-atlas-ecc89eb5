...
<Warning>
Temporal correlation requires log entries to have a parseable timestamp field. Zeal recognises `timestamp`, `ts`, `time`, and `@timestamp` automatically. Entries without a timestamp are included only as a best-effort fallback (when at least one anchor event exists).
</Warning>
